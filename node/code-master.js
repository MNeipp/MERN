...
const pkmnmultitype = pokemon.filter(p => p.types.length > 1)
const pkmnname = pokemon.map (p => p.name )
const pkmnnameid99 = pokemon.filter(p => p.id > 99).map(p => p.name)
const pkmnpoison = pokemon.filter(p => p.types.includes('poison')).map(p => p.name)
